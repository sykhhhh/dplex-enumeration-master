...
public:
	 ...
	 */
	void strong_reduce(int k, int l)
	{
		ui m_before = m;
		do
		{
			vertex_reduction(k, l);
			m_before = m;
			edge_reduction(k, l);
		} while (m_before > m);
	}
		...
		for (auto &vertex : vertices)
		{
			...
			for (ui i = 0; i < vertex.out_neighbors.size(); i++)
			{
				...
				for (ui w : vertices[v].out_neighbors)
				{
					if (vis[w])
					{
						removed = 0; // |N^{in}(u) \cap N^{out}(v)| >= 1, so we can not remove (u,v)
						break;
					}
				}
				if (!removed)
	 ...
	 */
	void vertex_reduction(int k, int l)
	{
	 ...
	 */
	void two_hop_reduction(int k, int l, int u)
	{
		ui prev_n;
		do
		{
			...
			if (k == l)
			{
				...
				for (auto &vertex : vertices)
				{
					...
					if (vertex.id == u)
						continue;
					bool u_to_v = has(u_vertex.out_neighbors, v);
					bool v_to_u = has(u_vertex.in_neighbors, v);
					if (u_to_v && v_to_u)
						continue;
					if (u_to_v) // u->v but not v->u: Nin(u) \cap Nout(v)>0
					{
						if (intersect_count(u_vertex.in_neighbors, vertex.out_neighbors) == 0)
						{
							vertex_removed[v] = 1;
							continue;
						}
					}
					else if (v_to_u) // v->u but not u->v: Nin(v) \cap Nout(u) > 0
					{
						if (intersect_count(u_vertex.out_neighbors, vertex.in_neighbors) == 0)
						{
							vertex_removed[v] = 1;
							continue;
						}
					}
					else // u v are not adjacent, 4 kinds of common neighbor
					{
						if (intersect_count(u_vertex.in_neighbors, vertex.out_neighbors) == 0)
						{
							vertex_removed[v] = 1;
						...
						if (intersect_count(u_vertex.out_neighbors, vertex.out_neighbors) == 0)
						{
							vertex_removed[v] = 1;
							continue;
						}
						if (intersect_count(u_vertex.in_neighbors, vertex.in_neighbors) == 0)
						{
							vertex_removed[v] = 1;
							continue;
						}
			...
			else
			{
				...
				for (auto &vertex : vertices)
				{
					...
					if (u_to_v || v_to_u)
						continue;
					// u v are not adjacent, we only care common out-neighbor
					if (intersect_count(u_vertex.out_neighbors, vertex.out_neighbors) == 0)
					{
						vertex_removed[v] = 1;
						continue;
