...
public:
    Prepro(Graph &_g, Output &_o, int paramK, int paramL,int paramQ) : g(_g), out(_o), k(paramK), l(paramL),q(paramQ)
    {
    }
    ~Prepro() {}
    ...
    void simple_reduce()
    {
        ...
        while (Q.size() && n >= k)
        {
            ...
            for (ui u : vertex.out_neighbors)
            {
                ...
                bool condition_2 = true; // u and all out-neighbors don't have bi-directed edges
                for (ui v : out_neighbors)
                    if (!no_directed[v] && !in_queue[v])
                    {
                        condition_2 = false;
                        break;
                    }
                if (!condition_2)
                    continue;
                // condition 1: | Nout2hop(u) \cap (Nin(u)) | = 0
                for (ui v : in_neighbors) // mark the in-neighbors
                    if (!in_queue[v])
                        vis[v] = 1;
                bool ok = 1; // ok <=> | Nout2hop(u) \cap (Nin(u)) | = 0
                for (ui v : g.vertices[u].out_neighbors)
                    if (vis[v])
                    {
                        ok = 0;
                ...
                for (ui v : in_neighbors) // clear the cache
                    vis[v] = 0;
                if (ok)
                {
                    Q.push(u);
